        ...
        */
        ...
        fn get_limit(limit_name: &str, single_print: bool, for_hard: bool) -> (String, String) {
            ...
            unsafe {
                ...
                let to_print = if for_hard { rlp.rlim_max }
    ...
    */
    ...
    pub fn eval_int(expression: Pairs<Rule>) -> i64 
    {
        ...
        .map_infix(|lhs: i64, op: Pair<Rule>, rhs: i64| match op.as_rule()
        {
            ...
            Rule::divide =>
            {
                if rhs == 0 { (lhs as f64 / 0.0) as i64 }
                else { (W(lhs) / W(rhs)).0 }
            }
    ...
    pub fn calculate( l:&str ) -> Result<Pairs<'_, Rule>, Error<Rule>> { Calculator::parse(Rule::calculation, l) }
}
    ...
    */
    pub mod dots
    {
        ...
        */
        ...
        impl<Term: Terminal> Completer<Term> for DotsCompleter {
            ...
            fn word_start(&self, l:&str, end: usize, _reader: &Prompter<Term>) -> usize {
        ...
        fn get_dot_file(l:&str) -> (String, String)
        {
            let args = parses::lines::line_to_plain_tokens(l);
            let dir = get::user_completer_dir();
            ...
            let sub_cmd = if (args.len() >= 3 && !args[1].starts_with('-')) || (args.len() >= 2 && !args[1].starts_with('-') && l.ends_with(' '))
            {
                args[1].as_str()
            }
        ...
        fn complete_dots(l:&str, word: &str) -> Vec<Completion> {
            let mut res = Vec::new();
            if l.trim().is_empty() {
                return res;
            }
            
            let (dot_file, sub_cmd) = get_dot_file(l);
    ...
    pub mod path
    {
        ...
        */
        ...
        fn needs_expand_home(l:&str) -> bool {
    ...
    pub fn for_make(l:&str) -> bool { regex::contains(l, r"^ *make ") }

    pub fn for_env(l:&str) -> bool 
    {
        regex::contains(l, r" *\$[_a-zA-Z0-9]*$")
    }

    pub fn for_ssh(l:&str) -> bool 
    {
        regex::contains(l, r"^ *(ssh|scp).* +[^ \./]+ *$")
    }

    pub fn for_cd(l:&str) -> bool 
    {
        regex::contains(l, r"^ *cd +")
    }

    pub fn for_bin(l:&str) -> bool 
    {
        regex::contains(l, r"(^ *(sudo|which|nohup)? *[a-zA-Z0-9_\.-]+$)|(^.+\| *(sudo|which|nohup)? *[a-zA-Z0-9_\.-]+$)")
    }

    pub fn for_dots(l:&str) -> bool 
    {
        let args = parses::lines::line_to_plain_tokens(l);
        let len = args.len();
        if len == 0 {
    ...
    impl<T:Terminals> Completer<T> for CicadaCompleter
    {
        fn complete(
            &self,
            word: &str,
        ...
        fn word_start(&self, l:&str, end: usize, _reader: &Prompter<T>) -> usize {
            escaped_word_start(&line[..end])
        }
    }

    pub fn escaped_word_start(l:&str) -> usize
    {
...
pub mod core
{
    ...
    */
    ...
    fn try_run_calculator(l:&str, capture: bool) -> Option<CommandResult> {
    ...
    pub fn run_calculator(l:&str) -> Result<String, &str> {
...
pub mod execute
{
    ...
    */
    ...
    pub fn line_to_tokens(sh: &mut Shell, l:&str) -> (Tokens, HashMap<String, String>)
    {
    ...
    pub fn run_proc(sh: &mut Shell, l:&str, tty: bool, capture: bool) -> CommandResult
    {
    ...
    pub fn run_with_shell(sh: &mut Shell, l:&str) -> CommandResult
    {
    ...
    pub fn run(l:&str) -> CommandResult
    {
...
pub mod highlight
{
    ...
    */
    ...
    {
        fn highlight(&self, l:&str) -> Vec<(Range<usize>, Style)>
        {
...
pub mod history
{
    ...
    */
    ...
    pub fn add_raw(sh: &shell::Shell, l:&str, status: i32, tsb: f64, tse: f64) 
    {
        let hfile = get_history_file();
        let history_table = get_history_table();
        if !Path::new(&hfile).exists() {
...
pub mod is
{
    ...
    */
    ...
    // pub fn is_env(l:&str) -> bool
    pub fn env(l:&str) -> bool
    {
        regex::contains(l, r"^[a-zA-Z_][a-zA-Z0-9_]*=.*$")
    }
    // pub fn is_arithmetic(l:&str) -> bool
    pub fn arithmetic(l:&str) -> bool
    {
        if !contains(l, r"[0-9]+")
        {
            return false;
        ...
        contains(l, r"^[ 0-9\.\(\)\+\-\*/\^]+[\.0-9 \)]$")
    }
    // pub fn is_shell_altering_command(l:&str) -> bool
    pub fn shell_altering_command(l:&str) -> bool
    {
        let l = l.trim();

        if contains(l, r"^[A-Za-z_][A-Za-z0-9_]*=.*$") { return true; }

        l.starts_with("alias ")
        || l.starts_with("export ")
        || l.starts_with("unalias ")
        || l.starts_with("unset ")
...
pub mod parses
{
    ...
    */
    ...
    pub mod lines
    {
        ...
        */
        pub fn line_to_plain_tokens(l:&str) -> Vec<String> 
        {
        ...
        pub fn line_to_cmds(l:&str) -> Vec<String> 
        {
            let mut result = Vec::new();
            let mut sep = String::new();
        ...
        pub fn parse_line(l:&str) -> LineInfo 
        {
            let mut result = Vec::new();
            if is::arithmetic(line) {
                for x in line.split(' ') {
    ...
    pub mod main
    {
        ...
        */
        ...
        fn is_prompt_item_char(c: char, token: &str) -> bool 
        {
            let s = c.to_string();
            if token.is_empty() {
                regex::contains(&s, r#"^[a-zA-Z_]$"#)
            }
        
        else
        {
                regex::contains(&s, r#"^[a-zA-Z0-9_]$"#)
            }
        }
...
pub mod scripts
{
    ...
    */
    ...
    fn expand_args(l:&str, args: &[String]) -> String 
    {
        let linfo = parses::lines::parse_line(line);
        let mut tokens = linfo.tokens;
    ...
    fn expand_line_to_toknes(l:&str, args: &[String], sh: &mut shell::Shell) -> types::Tokens 
    {
        let linfo = parses::lines::parse_line(line);
        let mut tokens = linfo.tokens;
        expand_args_in_tokens(&mut tokens, args);
...
pub mod shell
{
    ...
    */
    ...
    fn need_expand_brace(l:&str) -> bool 
    {
        regex::contains(line, r#"\{[^ "']*,[^ "']*,?[^ "']*\}"#)
    }
    ...
    fn should_do_dollar_command_extension(l:&str) -> bool 
    {
        regex::contains(line, r"\$\([^\)]+\)")
            && !regex::contains(line, r"='.*\$\([^\)]+\).*'$")
    }
    ...
    pub fn trim_multiline_prompts(l:&str) -> String 
    {
...
pub mod system
{
    ...
    pub mod common
    {
        ...
        */
        ...
        pub mod terminal
        {
            ...
            use mortal::signal::{Signal, SignalSet};
            use mortal::sys; */
            pub struct Terminal( pub super::sys::Terminal );
    ...
    pub mod interface
    {
        ...
        */
        ...
        /*
        use lineread::command::Command;
        use lineread::complete::Completer;
        use lineread::function::Function;
        use lineread::highlighting::Highlighter;
        use lineread::inputrc::Directive;
        use lineread::reader::{Read, Reader, ReadLock, ReadResult};
        use lineread::terminal::{DefaultTerminal, Signal, Terminal};
        use lineread::variables::Variable;
        use lineread::writer::{Write, Writer, WriteLock};
        */
    ...
    pub mod prompter
    {
        ...
        */
        ...
        use lineread::chars::{is_ctrl, is_printable, DELETE, EOF};
        use lineread::command::{Category, Command};
        use lineread::complete::Completion;
        use lineread::function::Function;
        use lineread::reader::{BindingIter, InputState, ReadLock, ReadResult};
        use lineread::table::{format_columns, Line, Table};
        ...
        use lineread::variables::VariableIter;
        use lineread::writer::{
            BLINK_DURATION, display_str,
            Digit, Display, HistoryIter, PromptType, Writer, WriteLock,
        };
        */
        pub struct Prompter<'a, 'b:'a, T:'b + Terminals>
        {
            pub read: &'a mut ReadLock<'b, T>,
            write: WriteLock<'b, T>,
        }
    }
    ...
    pub mod terminal
    {
        ...
        */
        ...
        pub trait TerminalReader<T:Terminals>
        {
            fn prepare(&mut self, block_signals: bool, report_signals: SignalSet) -> io::Result<T::PrepareState>;
            fn restore(&mut self, state: T::PrepareState) -> io::Result<()>;
            fn read(&mut self, buf: &mut Vec<u8>) -> io::Result<RawRead>;
            fn wait_for_input(&mut self, timeout: Option<Duration>) -> io::Result<bool>;
            unsafe fn prepare_with_lock(&mut self, lock: &mut dyn TerminalWriter<T>, block_signals: bool, report_signals: SignalSet) -> io::Result<T::PrepareState>;
            unsafe fn restore_with_lock(&mut self, lock: &mut dyn TerminalWriter<T>, state: T::PrepareState) -> io::Result<()>;
        }
        
        pub trait TerminalWriter<T:Terminals> 
        {
...
pub mod types
{
    ...
    */
    ...
    impl CommandLine
    {
        pub fn from_line(l:&str, sh: &mut shell::Shell) -> Result<CommandLine, String> {
            let linfo = parses::lines::parse_line(line);
            let mut tokens = linfo.tokens;
            shell::do_expansion(sh, &mut tokens);
            let envs = drain_env_tokens(&mut tokens);
